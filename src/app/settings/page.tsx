...
export default function SettingsPage() {
  ...
  // Default tote fields (array of strings)
  const [defaultToteFields, setDefaultToteFields] = useState<string[]>([]);
  const [newToteField, setNewToteField] = useState('');
  ...
  const fetchSettings = useCallback(async () => {
    try {
      ...
      try {
        ...
        setDefaultToteFields(Array.isArray(toteFields) ? toteFields : []);
      } catch {
        setDefaultToteFields([]);
      }
      try {
  ...
  const handleSave = async () => {
    ...
    try {
      const updatedSettings = {
        ...
        default_tote_fields: JSON.stringify(defaultToteFields),
  ...
  const addToteField = () => {
    const trimmed = newToteField.trim();
    if (trimmed && !defaultToteFields.includes(trimmed)) {
      setDefaultToteFields([...defaultToteFields, trimmed]);
      setNewToteField('');
    }
  };

  const removeToteField = (index: number) => {
    setDefaultToteFields(defaultToteFields.filter((_, i) => i !== index));
  };
        ...
        <div className="settings-section" data-testid="settings-default-tote-fields">
          <div className="settings-section-header">
            ...
            <div>
              ...
              <p className="settings-section-desc">
                Fields that are pre-populated when creating new totes. Add commonly used
                tote attributes here.
              </p>
            </div>
          </div>
          <div className="settings-section-body">
            <div className="settings-tag-list">
              {defaultToteFields.length === 0 && (
                <p className="settings-empty-hint">No default tote fields configured.</p>
              )}
              {defaultToteFields.map((field, index) => (
                <div key={index} className="settings-tag">
                  <span>{field}</span>
                  <button
                    className="settings-tag-remove"
                    onClick={() => removeToteField(index)}
            ...
            <div className="settings-add-row">
              <input
                ...
                value={newToteField}
                onChange={(e) => setNewToteField(e.target.value)}
                onKeyDown={(e) => {
                  if (e.key === 'Enter') {
                    e.preventDefault();
                    addToteField();
              ...
              <button
                ...
                onClick={addToteField}
                disabled={!newToteField.trim()}
              >
                <Plus size={16} />
                Add
              </button>
